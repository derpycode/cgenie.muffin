...
CONTAINS
  ...
  ! ****************************************************************************************************************************** !
  ! LOAD reggem 'goin' FILE OPTIONS
  SUBROUTINE sub_load_goin_reggem()
    ...
    ! local variables
    integer::ios
    ! read data_reggem file
    call check_unit(in,__LINE__,__FILE__)
    open(unit=in,file='data_REGGEM',status='old',action='read',iostat=ios)
    if (ios /= 0) then
       print*,'ERROR: could not open reggem initialisation namelist file'
       stop
    end if
    ! read in namelist and close data_reggem file
    read(UNIT=in,NML=ini_reggem_nml,IOSTAT=ios)
    if (ios /= 0) then
       print*,'ERROR: could not read reggem namelist'
       stop
    else
  ...
  END SUBROUTINE sub_load_goin_reggem
  ! ****************************************************************************************************************************** !


  ! ****************************************************************************************************************************** !
  ! Subroutine: sub_load_reggem_restart
  !
  ...
  SUBROUTINE sub_load_reggem_restart()
    ...
    !call check_iostat(ios,__LINE__,__FILE__)
    READ(unit=in,fmt='(i6)') ncout2d_ntrec_gg                             
    close(unit=in)
  end SUBROUTINE sub_load_reggem_restart
  ! ****************************************************************************************************************************** !
  ...
  SUBROUTINE sub_init_phys_reg()
    ...
    DO i=1,n_i
       DO j=1,n_j
          phys_reg(ipr_lat,i,j)  = (180.0/const_pi)*ASIN(loc_s(j)) ! lattitudes (degrees; +=N, -=S) of mid-points of grid cells
          phys_reg(ipr_latn,i,j)  = (180.0/const_pi)*ASIN(loc_sv(j)) ! lattitudes (degrees) of the northerly edges of grid cells
          phys_reg(ipr_lon,i,j)  = (360.0/real(n_i))*(real(i)-0.5) + par_grid_lon_offset ! longitudes (degrees) of mid-points of grid cells
          phys_reg(ipr_lone,i,j)  = (360.0/real(n_i))*(real(i)) + par_grid_lon_offset ! longitudes (degrees) of easterly edges of grid cells
          phys_reg(ipr_dlat,i,j) = (180.0/const_pi)*(ASIN(loc_sv(j)) - ASIN(loc_sv(j-1))) ! heights of grid cells in degrees latitude
          phys_reg(ipr_dlon,i,j) = (360.0/real(n_i)) ! widths of grid cells in degreees longitude
          phys_reg(ipr_A,i,j)    = 2.0*const_pi*(const_rEarth**2)*(1.0/real(n_i))*(loc_sv(j) - loc_sv(j-1)) ! areas of grid cells
          phys_reg(ipr_rA,i,j)   = 1.0/phys_reg(ipr_A,i,j) ! reciprocals of areas
       END DO
    END DO
  END SUBROUTINE sub_init_phys_reg
  ! ****************************************************************************************************************************** !

  ! taken from biogem_data (where is is sub_init_phys_ocnatm):
  ! ****************************************************************************************************************************** !
  ! INITIALIZE 'PHYSICS' - OCEAN-reggem INTERFACE
  SUBROUTINE sub_init_phys_ocnreg()
    ! local variables
    INTEGER::i,j
    ...
    DO i=1,n_i
       DO j=1,n_j
          !          phys_ocnreg(ipor_lat,i,j)  = (180.0/const_pi)*ASIN(goldstein_s(j))
          !          phys_ocnreg(ipor_lon,i,j)  = (360.0/n_i)*(real(i)-0.5) + par_grid_lon_offset
          !          phys_ocnreg(ipor_dlat,i,j) = (180.0/const_pi)*(ASIN(goldstein_sv(j)) - ASIN(goldstein_sv(j-1)))
          !          phys_ocnreg(ipor_dlon,i,j) = (360.0/n_i)
          !          phys_ocnreg(ipor_A,i,j)    = 2.0*const_pi*(const_rEarth**2)*(1.0/n_i)*(goldstein_sv(j) - goldstein_sv(j-1))
          !          phys_ocnreg(ipor_rA,i,j)   = 1.0/ phys_ocnreg(ipor_A,i,j)
          !          IF (n_k >= goldstein_k1(i,j)) THEN
          !             phys_ocnreg(ipor_seaice,i,j) = 0.0
          !             phys_ocnreg(ipor_u,i,j)      = 0.0
          !             phys_ocnreg(ipor_mask_ocn,i,j) = 1.0
          !          END IF
       END DO
    END DO
    ...
    !    end if
  END SUBROUTINE sub_init_phys_ocnreg
  ! ****************************************************************************************************************************** !     
  ...
  SUBROUTINE sub_ini_output()
    ...
    DO i=1,n_outputs    
       IF ((output_counter_0d.eq.1).AND.(((opt_append_data.eqv..FALSE.).OR.(ctrl_continuing.eqv..FALSE.)))) THEN
          ...
          open(20,file=TRIM(par_outdir_name)//'reggem_series_'//TRIM(time_series_names(i))//string_results_ext, &
  ...
  SUBROUTINE sub_output_0d(dum_n_outputs,dum_output_divisions,dum_outputs,dum_output_descriptions,dum_time_series_names)
    ...
    DO i=1,dum_n_outputs    
       ...
       call check_unit(20,__LINE__,__FILE__)
       open(20,file=TRIM(par_outdir_name)//'reggem_series_'//TRIM(dum_time_series_names(i))//string_results_ext, &
    ...
    call check_unit(20,__LINE__,__FILE__)
    open(20,file=TRIM(par_outdir_name)//'RegGem_DATA_GLOBAL_year_'//TRIM(year_text)//string_results_ext &
