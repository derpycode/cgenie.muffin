!
! File: reggem_data_netCDF.f90
! 
! Description: this module contains the subroutines for generating netCDF output from reggem.
!
! Uses:
!
! - <gem_netcdf.f90>
...
CONTAINS
  ...
  SUBROUTINE reggem_netcdf(dum_temp,dum_CO2,dum_runoff,dum_photo,dum_respveg,dum_loc_P,&
       & dum_force_flux_weather_a_land,dum_force_flux_weather_o_land,&
       & dum_force_flux_weather_o_ocean)

    ! dummy variables
    !(NOTE: output for temp, runoff and productivity is that used in reggem calculations, so may have been calibrated if option set)
    ...
    REAL,INTENT(in)                 :: dum_loc_P(n_i,n_j)                                       ! Productivity as calculated for reggem from ENTS inputs
    ...
    CALL sub_save_netcdf_2d_gg(dum_temp,dum_CO2,dum_runoff,dum_photo,dum_respveg,dum_loc_P, &                      
    ...
    call sub_closefile(ncout2d_iou_gg)
    ncout2d_ntrec_gg = ncout2d_ntrec_gg + 1

  END SUBROUTINE reggem_netcdf
  ...
  SUBROUTINE sub_save_netcdf_2d_gg(dum_temp,dum_CO2,dum_runoff,dum_photo,dum_respveg,dum_loc_P,&
       & dum_force_flux_weather_a_land,dum_force_flux_weather_o_land,&
       & dum_force_flux_weather_o_ocean)
    ...
    !-----------------------------------------------------------------------
    loc_iou   = ncout2d_iou_gg
    loc_ntrec = ncout2d_ntrec_gg
    loc_mask_surf(:,:) = 1.0
    loc_mask_ocean(:,:) = 0.0
    where ( landmask(:,:) .eq. 0 )
       loc_mask_surf = 0.0
    ...
    ! ############################################################################################################################ !
    !-----------------------------------------------------------------------
  END SUBROUTINE sub_save_netcdf_2d_gg
  ! ****************************************************************************************************************************** !


  ! ****************************************************************************************************************************** !
  ! INITIALIZE netCDF
  SUBROUTINE sub_init_netcdf_gg(dum_name, dum_iou)
    !-----------------------------------------------------------------------
    !       dummy arguments
    !-----------------------------------------------------------------------
    ...
    call sub_putglobal (dum_iou, dum_name, loc_title, string_ncrunid_gg, loc_timunit)
    !-----------------------------------------------------------------------
    !       define dimensions
    !-----------------------------------------------------------------------
...
!!$       !-----------------------------------------------------------------------
    ...
    !-----------------------------------------------------------------------
  END SUBROUTINE sub_init_netcdf_gg
  ! ****************************************************************************************************************************** !
  ...
  SUBROUTINE sub_save_netcdf(dum_yr)
    ...
    loc_yu_e(:)        = 0.0
    ! 
    loc_name = string_ncout2d_gg
    !print*,'loc_name = ',string_ncout2d_gg
    loc_iou = ncout2d_iou_gg
    !print*,'loc_iou = ',ncout2d_iou_gg
    loc_ntrec = ncout2d_ntrec_gg
    !print*,'loc_ntrec = ',ncout2d_ntrec_gg
    !-----------------------------------------------------------------------
    ...
    if(.not. loc_defined) then
       ...
       call sub_putvar1d ('lon', loc_iou, n_i, loc_ntrec, n_i, &
            & phys_reg(ipr_lon,:,1), loc_c1, loc_c0)
       call edge_maker (1, loc_lon_e, phys_reg(ipr_lon,:,1), &
            & phys_reg(ipr_lone,:,1), phys_reg(ipr_dlon,:,1), n_i)
       ...
       call edge_maker (2, loc_xu_e, phys_reg(ipr_lon,:,1), &
            & phys_reg(ipr_lone,:,1), phys_reg(ipr_dlon,:,1), n_i)
       ...
       call sub_putvar1d ('lat', loc_iou, n_j, loc_ntrec, n_j, &
            & phys_reg(ipr_lat,1,:), loc_c1, loc_c0)
       call edge_maker (1, loc_lat_e, phys_reg(ipr_lat,1,:), &
            & phys_reg(ipr_latn,1,:), phys_reg(ipr_dlat,1,:), n_j)
       ...
       call edge_maker (2, loc_yu_e, phys_reg(ipr_lat,1,:), &
            & phys_reg(ipr_latn,1,:), phys_reg(ipr_dlat,1,:), n_j)
    ...
    ncout2d_ntrec_gg = loc_ntrec
    ncout2d_iou_gg = loc_iou
