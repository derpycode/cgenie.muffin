...
! File: initialise_reggem.f90
!
! Contains initialisation subroutine for reggem, as called in <initialise_genie.F> through <genie_ini_wrappers.f90>
!
! Subroutine: initialise_reggem
!
! Initialisation subroutine for reggem, called in <initialise_genie.F> through <genie_ini_wrappers.f90>
!
! Uses:
!
!  - <reggem_lib.f90>
!  - <reggem_data.f90>
!  - <reggem_box.f90>
!  - <reggem_data_netCDF.f90>
!
! Calls:
!
! - <sub_load_goin_reggem>
! - <sub_init_phys_reg>
! - <sub_init_netcdf_gg>
! - <sub_load_reggem_restart>
! - <sub_init_netcdf_gg>
...
! dum_sfxreg - rocks-surface ocean tracer composition; reg grid
! dum_sfxsumreg1 - rocks-surface ocean fluxes; integrated, ocn grid
...
subroutine initialise_reggem( &
  ...
  REAL,dimension(n_ocn,n_i,n_j),intent(inout)::dum_sfxreg     ! rocks-surface tracer composition;reg grid
  REAL,dimension(n_ocn,n_i,n_j),intent(inout)::dum_sfxsumreg1 ! rocks-surface fluxes; integrated, ocn grid
  ...
  print*,'======================================================='
  print*,' >>> Initialising REGGEM weathering module ...'

  ! *** load goin information ***
  CALL sub_load_goin_reggem()

  ! *** initialize reggem ***
  CALL sub_init_phys_reg()

  ! *** setup for netcdf output  ***
  if (debug_init > 1) print*, 'initialize netCDF'
  string_ncout2d_gg  = TRIM(par_outdir_name)//'fields_reggem_2d.nc' !note: this needs to be less than 100 characters
  if (debug_init > 1) print*, 'netcdf ouput file: ',TRIM(string_ncout2d_gg)
  ! initialise 2d netcdf files
  IF (ctrl_continuing.AND.opt_append_data) THEN
     call sub_load_reggem_restart()
  ELSE   
     ncout2d_ntrec_gg = 0
     call sub_init_netcdf_gg(trim(string_ncout2d_gg),loc_iou)
  ENDIF
  if (debug_init > 1) print*, 'netcdf record number: ',ncout2d_ntrec_gg
  ...
  tstep_count = 0
  tsteps_per_year = conv_yr_s/(dum_genie_timestep*kocn_loop*conv_kocn_kreggem)
...
end subroutine initialise_reggem
! ******************************************************************************************************************************** !
