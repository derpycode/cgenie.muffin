...
% 22Apr16 (original code: 5Mar16); edited 22Jan17 to make idxSt defined by
% other variables (rather than fixed factor)
% PM Hull
%------------%------------%------------%------------%------------
...
thermy=thermoMean .* gappy10;   % thermocline foram: 10cm (i.e., 4kyr) gap
...
%------------%------------%------------%------------%------------%
%% Calculate probability of overlap for mixed layer (example only)

% saving structures for percent samples w/overlapping indivi.
bootOverlap=1000;
surfOver4=zeros(length(winStart:winStop),bootOverlap);surfOver20=surfOver4; surfOver100=surfOver4;

% remove nans from emixed file structure
sCell(cellfun(@(sCell) any(isnan(sCell)),sCell)) = [];

% %Runs w/samples w/pre- and post-event individuals
for k = 1:length(tCell)
    for x= 1:bootOverlap
        surfOver4(k,x) = (range(datasample(sCell{k},4,'Replace', false))>1.5);
        surfOver20(k,x) = (range(datasample(sCell{k},20,'Replace', false))>1.5);
        surfOver100(k,x) = (range(datasample(sCell{k},100,'Replace', false))>1.5);
    end
end

%calculate percent of samples showing overlap and save
surfOverlap = zeros(length(winStart:winStop), 3); % surface foraminifera
surfOverlap(:,1)=sum(surfOver4,2)/bootOverlap;
surfOverlap(:,2)=sum(surfOver20,2)/bootOverlap;
surfOverlap(:,3)=sum(surfOver100,2)/bootOverlap;
%------------%------------%------------%------------%------------%
%% Save results
