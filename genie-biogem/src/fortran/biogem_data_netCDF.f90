       ...
       if (ocn_select(io_PO4).and.ocn_select(io_Fe2)) then 
          if (sed_select(is_FeOOH)) then 
             ...
             call sub_adddef_netcdf(loc_iou,4,'dPO4 on FeOOH','dPO4 adsorption on FeOOH', &
                  & trim(loc_unitsname),const_real_zero,const_real_zero)
             call sub_putvar3d_g('dPO4 on FeOOH',loc_iou,n_i,n_j,n_k,loc_ntrec,loc_ijk(:,:,:),loc_mask)
             
             loc_ijk(:,:,:) = const_real_null
             loc_unitsname = 'mol kg-1'
             loc_ijk(:,:,:) = bio_part(is_FeOOH,:,:,:)
             call sub_adddef_netcdf(loc_iou,4,'bio_part_FeOOH_chk','bio_part_FeOOH_chk', &
                  & trim(loc_unitsname),const_real_zero,const_real_zero)
             call sub_putvar3d_g('bio_part_FeOOH_chk',loc_iou,n_i,n_j,n_k,loc_ntrec,loc_ijk(:,:,:),loc_mask)
          endif 
          if (sed_select(is_POM_FeOOH)) then 
             ...
             call sub_adddef_netcdf(loc_iou,4,'dPO4 on POM-FeOOH','dPO4 adsorption on POM assoc-FeOOH', &
                  & trim(loc_unitsname),const_real_zero,const_real_zero)
             call sub_putvar3d_g('dPO4 on POM-FeOOH',loc_iou,n_i,n_j,n_k,loc_ntrec,loc_ijk(:,:,:),loc_mask)
             
             loc_ijk(:,:,:) = const_real_null
             loc_unitsname = 'mol kg-1'
             loc_ijk(:,:,:) = bio_part(is_POM_FeOOH,:,:,:)
             call sub_adddef_netcdf(loc_iou,4,'bio_part_POM_FeOOH_chk','bio_part_POM_FeOOH_chk', &
                  & trim(loc_unitsname),const_real_zero,const_real_zero)
             call sub_putvar3d_g('bio_part_POM_FeOOH_chk',loc_iou,n_i,n_j,n_k,loc_ntrec,loc_ijk(:,:,:),loc_mask)
         endif 
         
         loc_ijk(:,:,:) = const_real_null
         loc_unitsname = 'mol kg-1'
         loc_ijk(:,:,:) = ocn(io_PO4,:,:,:)
         call sub_adddef_netcdf(loc_iou,4,'ocn_PO4_chk','checking oceanic PO4', &
              & trim(loc_unitsname),const_real_zero,const_real_zero)
