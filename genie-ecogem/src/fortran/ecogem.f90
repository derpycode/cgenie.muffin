...
subroutine ecogem(          &
  ...
  REAL,DIMENSION(n_i,n_j,n_k)              ::temp,omega,gamma
...
!  REAL,DIMENSION(iomax+iChl,npmax,n_i,n_j,n_k) ::biomass_flux
  ...
  REAL,DIMENSION(n_i,n_j,n_k)              ::ocn_CO2 !for like-Biogem POC Cfrac isotopes
  ...
  !temp(:,:)=merge(temp-273.15,temp,temp.ne.0.0) ! convert to degrees C when temperature ~= absolute zero
...
! Dissolved nutrients from BIOGEM
 ...
 !ckc testing
  ...
  END IF
              !print*, 'plankiso all  ', plankiso(iCarb13C,1,:,:,:) !plankiso still fine here
              !print*, 'plankton all    ', plankton(iCarb,1,:,:,:) 
  ! ---------------------------------------------------------- !
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc 
            ...
            !print*, 'plankton all  ', plankton(iCarb,:,i,j,k)
            !print*, 'plankiso all  ', plankiso(iCarb13C,:,i,j,k) !now plankiso a mess
            !print*, 'biociso   ' , BioCiso(1)
            end if
...
! v ********************************** v
! v *** UPDATE CARBONATE CHEMSITRY *** v
! v ********************************** v
          ...
          !ckc added by kc, calc carbisor of ocean waters, as Biogem
           IF (ocn_select(io_DIC_13C)) then
               call sub_calc_carb_r13C(              &
                            ...
                            & eco_carbisor(     :,i,j,k)  &
                            & )
           !print*,'eco_carbisor' ,eco_carbisor(:,i,j,k) !I think this is little r
           end IF
...
! call ecophysiological subroutines
            ...
            !print*, "up inorgiso iDIC_13C  ", up_inorgiso(iDIC_13C,:)  !fine
            !print*, "up inorg iDIC  ", up_inorg(iDIC,:)
            !call exudation
            !call sinking
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            ...
            enddo ! ii
                        !print*, "dnutrientdt b4 resp  ", dnutrientdt(iDIC)
                        !print*, "BioC b4 resp   ", BioC(:)
            ...
            if (c13trace) then
              ...
              enddo
            !print*, "dnutisodt b4 resp  ", dnutisodt(iDIC_13C)
            !print*, "BioCiso b4 resp   ", BioCiso(:) !seems fine when biociso forced
            ...
            !print*, "dnutrientdt  ", dnutrientdt(iDIC)
            !print*, "dnutisodt  ", dnutisodt(iDIC_13C) !this is negetive while dnutrientdt is poitive!
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            ...
            if (chlquota) dbiomassdt(iChlo,:) = chlsynth(:)
                !print*, 'dbiomassdt b4 graz   ', dbiomassdt(iCarb,1)
            do io=1,iomax+iChl
              ! MORTALITY
                ! Apply mortality
            ...
            if (c13trace) then
              ...
              dbioisodt(iCarb13C,:) = dbioisodt(iCarb13C,:) + up_inorgiso(iDIC_13C ,:) * BioCiso(:)
                !print*, 'biociso   ' , BioCiso(1)
              !print*, 'dbioisodt b4 graz   ' , dbioisodt(iCarb13C,1) seems ok
              ...
              !print*, 'dbioisodt b4 graz   ' , dbioisodt(iCarb13C,1) !its negetive here!!
              ! GRAZING INTERACTIONS
              ...
              !print*, 'dbioisodt   ' , dbioisodt(iCarb13C,1) !this is a problem, now almost same as biomassdt iCarb
              !print*, 'dbiomassdt   ', dbiomassdt(iCarb,1)
            end if 
            !******************************************************************************************************
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            ! ORGANIC MATTER
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            ...
            do io=1,iomax
              ...
              !print*,'loc_biomass       ', loc_biomass(iCarb,:)
              ...
              !end/start
            enddo
            !print*, 'orbGraz    ', orgbGraz(iCarb,:)
            !print*, 'org afterGraz   ', dorgmatdt(iCarb,:)
            !print*, 'ratioGraz    ', ratioGraz(iCarb,:)
            ...
            if (c13trace) then
              ...
              ! unassimilated grazing
            !print*, ' isoorg(iCarb13C,1)', dorgmatisodt(iCarb13C,1) !bigger than dorgmatdt!
            !print*, ' org(iCarb,1)         ', dorgmatdt(iCarb,1)
            !print*,' ratioGraz for org   ',  ratioGraz(iCarb,:) 
            !print*,'loc-bioiso    ', loc_bioiso(iCarb13C,:)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
			...
			!print*,'dbiomassdt   ', dbiomassdt(iCarb,1)
            !ckc need to update isotopes here too
            if (c13trace) then
              nutiso(:,i,j,k)        = loc_nutiso(:)          + dnutisodt(:)    * dum_dts/real(nsubtime)
              plankiso(:,:,i,j,k)    = loc_bioiso(:,:)        + dbioisodt(:,:)  * dum_dts/real(nsubtime)
            !print*, "dnutiso  ", nutiso(iDIC_13C,:,:,:)
			!print*,'dbioisodt   ', dbioisodt(iCarb13C,1) !too big compared to dbiomassdt
            end if
            ! ---------------------------------------------------------- !
            ...
            !print*, "dnutiso_flux  ", nutiso_flux(iDIC_13C,:,:,:)
            end if
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            ...
            !ckc c13 only for DOC and POC, not during uptake (as Biogem)
              Corg_frac = 25.0 !as par_d13C_DIC_Corg_ef
              !PDBstnd = 0.0112372 !PDB standard d13C  
              !print*, "Corg_frac", Corg_frac
              dorgmatisodt(iCarb13C,:)  = 0.0 
              dnutisodt(iCarb13C) = 0.0
              ! POC_Rfrac is big R
              POC_Rfrac(i,j,k) = fun_Corg_Rfrac(loc_ocn(io_T,i,j,k),ocn_CO2(i,j,k),&
                           & eco_carbisor(ici_CO2_r13C,i,j,k),Corg_frac,.false.) 
              ! apply to carbon fluxes    
              dorgmatisodt(iCarb13C,:)  = dorgmatdt(iCarb,:) * POC_Rfrac(i,j,k) 
              dnutisodt(iCarb13C)       = - sum(dorgmatisodt(iCarb13C,:))  
              !print*, 'dnutisodt calc from  orgmat  ', dnutisodt(iCarb13C)
			  ...
			  !print*, 'nutiso flux calc from  orgmat  ', nutiso_flux(:,i,j,k)
            end if
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
  ...
  !print*, "dum DIC13C  ", dum_egbg_sfcdiss(io_DIC_13C,:,:,:)
  !  Organic matter
  ! Dissolved
