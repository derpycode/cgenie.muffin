...
CONTAINS
  ...
  ! ****************************************************************************************************************************** !
  ! EXPAND TO VERBOSE INDEX FORMAT
  FUNCTION fun_conv_lslo2sedocn(dum_ls_lo)
    ! -------------------------------------------------------- !
    ! DUMMY ARGUMENTS
    ! -------------------------------------------------------- !
    real,dimension(n_l_ocn,n_l_sed),INTENT(in)::dum_ls_lo      !
    ! -------------------------------------------------------- !
    ! RESULT VARIABLE
    ! -------------------------------------------------------- !
    real,dimension(n_ocn,n_sed)::fun_conv_lslo2sedocn          !
    ! -------------------------------------------------------- !
    ! DEFINE LOCAL VARIABLES
    ! -------------------------------------------------------- !
    INTEGER::lo,ls
    real,dimension(n_ocn,n_sed)::loc_sedocn                    !
    ! -------------------------------------------------------- !
    ! INITIALIZE
    ! -------------------------------------------------------- !
    loc_sedocn(:,:) = 0
    ...
    ! TRANSFORM INDICES
    ! -------------------------------------------------------- !
    ! re-index array to verbose tracer format
    do ls=1,n_l_sed
       do lo=1,n_l_ocn
          if (abs(dum_ls_lo(lo,ls)) > const_real_nullsmall) then
             loc_sedocn(l2io(lo),l2is(ls)) = dum_ls_lo(lo,ls)
          end if
       end do
    end do
    ! return function result
    fun_conv_lslo2sedocn = loc_sedocn
    ! -------------------------------------------------------- !
    ! END
    ! -------------------------------------------------------- !
  END FUNCTION fun_conv_lslo2sedocn
  ! ****************************************************************************************************************************** !
